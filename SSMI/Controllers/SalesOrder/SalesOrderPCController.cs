...
namespace SSMI.Controllers.SalesOrder
{
    [SessionExpire]
    [Authorize]
    public class SalesOrderPCController : Controller
    {
        // GET: SalesOrderPC
        ...
        public ActionResult Details(long id)
        {
            ...
            {
                Voucher = voucher ?? new Tally_Voucher(),
                InventoryLines = inv,
                LedgerLines = led
            };
        ...
        public JsonResult GetAjaxData(JQueryDataTableParamModel param)
        {
            ...
            q = q.Where(v => v.RegstrId == 1);
            q = q.Where(v => v.VoucherType == "Sales Order");
            q = q.Where(v => v.VoucherDate.HasValue && v.VoucherDate.Value >= sd && v.VoucherDate.Value <= ed);
            q = q.Where(v => v.PreSyncStatus > 0); // Filter for pre-closed orders
